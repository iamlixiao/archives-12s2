...
void KAZE::Allocate_Memory_Evolution(void)
{
    ...
    {        
        for( int j = 0; j <= nsublevels-1; j++ )
        {
             ...
             aux2.Lx  = cv::gpu::GpuMat(aux.Lx);
             aux2.Ly  = cv::gpu::GpuMat(aux.Ly);

             aux2.Lxx = cv::gpu::GpuMat(aux.Lxx);
             aux2.Lxy = cv::gpu::GpuMat(aux.Lxy);
             aux2.Lyy = cv::gpu::GpuMat(aux.Lyy);
             aux2.Lflow = cv::gpu::GpuMat(aux.Lflow);
             aux2.Lt  = cv::gpu::GpuMat(aux.Lt);
             aux2.Lsmooth = cv::gpu::GpuMat(aux.Lsmooth);
             aux2.Lstep = cv::gpu::GpuMat(aux.Lstep);
             aux2.Ldet = cv::gpu::GpuMat(aux.Ldet);

             aux2.esigma = aux.esigma;
             aux2.etime = aux.etime;
             aux2.sigma_size = aux.sigma_size;

             aux2.octave = i;
             aux2.sublevel = j;
             gpuevolution.push_back(aux2);
        }
    }    
...
int KAZE::Create_Nonlinear_Scale_Space(const cv::Mat &img)
{
    ...
    {
        cv::gpu::GpuMat t1(evolution[i-1].Lt),t2(evolution[i].Lsmooth);
        Gaussian_2D_Convolution_GPU(t1,t2,0,0,sderivatives);
        t1.download(evolution[i-1].Lt);
        t2.download(evolution[i].Lsmooth);

        // Compute the Gaussian derivatives Lx and Ly
        cv::gpu::GpuMat t3(evolution[i].Lsmooth),t4(evolution[i].Lx),t5(evolution[i].Ly);
        Image_Derivatives_Scharr_GPU(t3,t4,1,0);
        Image_Derivatives_Scharr_GPU(t3,t4,0,1);
        t3.download(evolution[i].Lsmooth);
        t4.download(evolution[i].Lx);
        t5.download(evolution[i].Ly);
...
int KAZE::Create_Nonlinear_Scale_Space_GPU(const cv::gpu::GpuMat &img)
{
    if( verbosity == true ){
        std::cout << "\n> Creating nonlinear scale space." << std::endl;
    }
    ...
    img.copyTo(gpuevolution[0].Lt);
    Gaussian_2D_Convolution_GPU(gpuevolution[0].Lt,gpuevolution[0].Lt,0,0,soffset);
    Gaussian_2D_Convolution_GPU(gpuevolution[0].Lt,gpuevolution[0].Lsmooth,0,0,sderivatives);

    // Firstly compute the kcontrast factor
    cv::Mat tk;
    gpuevolution[0].Lt.download(tk);
    Compute_KContrast(tk,KCONTRAST_PERCENTILE);
    gpuevolution[0].Lt.upload(tk);
    ...
    // Now generate the rest of evolution levels
    for( unsigned int i = 1; i < gpuevolution.size(); i++ )
    {
        Gaussian_2D_Convolution_GPU(gpuevolution[i-1].Lt,gpuevolution[i].Lsmooth,0,0,sderivatives);

        // Compute the Gaussian derivatives Lx and Ly
        Image_Derivatives_Scharr_GPU(gpuevolution[i].Lsmooth,gpuevolution[i].Lx,1,0);
        Image_Derivatives_Scharr_GPU(gpuevolution[i].Lsmooth,gpuevolution[i].Ly,0,1);


        // Compute the conductivity equation
            for(int i=0;i<evolution.size();++i){
                tevolution aux=evolution[i];
                tgpuevolution aux2=gpuevolution[i];
                aux2.Lx.download(aux.Lx);
                aux2.Ly.download(aux.Ly);

                aux2.Lxx.download(aux.Lxx);
                aux2.Lxy.download(aux.Lxy);
                aux2.Lyy.download(aux.Lyy);
                aux2.Lflow.download(aux.Lflow);
                aux2.Lt.download(aux.Lt);
                aux2.Lsmooth.download(aux.Lsmooth);
                aux2.Lstep.download(aux.Lstep);
                aux2.Ldet.download(aux.Ldet);


                aux.esigma = aux2.esigma;
                aux.etime = aux2.etime;
                aux.sigma_size = aux2.sigma_size;

                aux.octave = aux2.octave;
                aux.sublevel = aux2.sublevel;
            }
            if( diffusivity == 0 )
            {
                PM_G1(evolution[i].Lsmooth,evolution[i].Lflow,evolution[i].Lx,evolution[i].Ly,kcontrast);
...
void KAZE::Compute_Multiscale_Derivatives(void)
{
    ...
    for( int i = 0; i < evolution.size(); i++ )
    {
        if( verbosity == true )
        {
            std::cout << "--> Multiscale derivatives. Sigma ("<< i <<"): " << evolution[i].sigma_size << ". Thread: " << 5 << std::endl;
        }
