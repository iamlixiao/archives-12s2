...
typedef struct
{
    cv::gpu::GpuMat Lx, Ly;         // 一阶微分图像（First order spatial derivatives）
    cv::gpu::GpuMat Lxx, Lxy, Lyy;  // 二阶微分图像（Second order spatial derivatives）
    cv::gpu::GpuMat Lflow;   // 传导图像（Diffusivity image）
    cv::gpu::GpuMat Lt;      // 进化图像（Evolution image）
    cv::gpu::GpuMat Lsmooth; // 平滑图像（Smoothed image）
    cv::gpu::GpuMat Lstep;   // 进化步长更新矩阵（Evolution step update）（！！实际未被使用！！）
    cv::gpu::GpuMat Ldet;    // 检测响应矩阵（Detector response）
    float etime;     // 进化时间（Evolution time）
    float esigma;    // 进化尺度（Evolution sigma. For linear diffusion t = sigma^2 / 2）
    float octave;    // 图像组（Image octave）
    float sublevel;  // 图像层级（Image sublevel in each octave）
