...
void Gaussian_2D_Convolution_GPU(const cv::gpu::GpuMat &src, cv::gpu::GpuMat &dst, unsigned int ksize_x, unsigned int ksize_y, float sigma)
{if( sigma > ksize_x || sigma > ksize_y || ksize_x == 0 || ksize_y == 0 )
    {
        ksize_x = ceil(2.0*(1.0 + (sigma-0.8)/(0.3)));
        ksize_y = ksize_x;
    ...
    // Perform the Gaussian Smoothing with border replication
    cv::gpu::GaussianBlur(src,dst,cv::Size(ksize_x,ksize_y),sigma,sigma,cv::BORDER_REPLICATE);
}


void Image_Derivatives_SD_GPU(const cv::gpu::GpuMat &src, cv::gpu::GpuMat &dst, unsigned int xorder, unsigned int yorder)
{
    unsigned int norder_x = xorder;
    unsigned int norder_y = yorder;
    int left = 0, right = 0, up = 0, down = 0;
    ...
    // Initialize the destination image
    dst=cv::gpu::GpuMat(cv::Mat::zeros(dst.rows,dst.cols,CV_32F));

    // Create an auxiliary image
    cv::gpu::GpuMat aux(dst.rows,dst.cols,CV_32F);
    src.copyTo(aux);
    ...
    while( norder_x != 0 )
    {
        for(int j=0;j<aux.cols;++j){
            ...
            cv::gpu::GpuMat a=dst.col(j);
            cv::Mat tb=cv::Mat::ones(cv::Size(aux.col(0).size()),aux.type())*0.5;
            std::cout<<"tb.at<double>(0,0)"<<endl;
            cv::gpu::multiply(aux.col(right),cv::gpu::GpuMat(tb),a);
            cv::gpu::subtract(dst.col(j),aux.col(left),a);
        }
    ...
    while( norder_y != 0 )
    {
        for(int j=0;j<aux.rows;++j){
            ...
            cv::gpu::GpuMat a=dst.row(j);
            cv::gpu::multiply(aux.row(down),cv::Scalar::all(0.5),a);
            cv::gpu::subtract(dst.col(j),aux.row(up),a);
        }
...
void Image_Derivatives_Scharr_GPU(const cv::gpu::GpuMat &src, cv::gpu::GpuMat &dst, unsigned int xorder, unsigned int yorder)
{
    cv::gpu::Scharr(src,dst,CV_32F,xorder,yorder,1,0,cv::BORDER_DEFAULT);
}


void PM_G2_GPU(const cv::gpu::GpuMat, cv::gpu::GpuMat &dst, cv::gpu::GpuMat &Lx, cv::gpu::GpuMat &Ly, float k)
{
    int N = Lx.rows * Lx.cols;
    float lx = 0.0, ly = 0.0, k2 = k*k;
...
void PM_G2_GPU(const cv::gpu::GpuMat &src, cv::gpu::GpuMat &dst, cv::gpu::GpuMat &Lx, cv::gpu::GpuMat &Ly, float k)
{
    int N = Lx.rows * Lx.cols;
    float lx = 0.0, ly = 0.0, k2 = k*k;
